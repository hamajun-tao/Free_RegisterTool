...
class KiroRegister:
    ...
    def _handle_desktop_auth_page(self, page: Page, email: str = "", pwd: str = ""):
        ...
        for label in ("Allow", "Continue", "Authorize"):
            ...
            try:
                # 方法 3: 使用精确文本匹配
                button = page.locator(f'text="{label}"').first
                if button.count() > 0:
                    self.log(f"桌面授权页点击 {label} (exact text) ...")
        ...
        # 如果所有方法都失败，记录当前页面状态
        try:
            self.log(f"桌面授权页未找到授权按钮，当前 URL: {page.url}")
            buttons = page.locator('button').all_text_contents()
            if buttons:
                self.log(f"当前页面按钮: {buttons}")
        except Exception:
            pass
        ...
        try:
            ...
            if not allow_clicked:
                try:
                    ...
                    if allow_btn.count() > 0:
                        self.log("点击 Allow 授权应用 (role)...")
                        allow_btn.wait_for(state="visible", timeout=10000)
                        allow_btn.click(timeout=5000)
                        allow_clicked = True
                        self._human_sleep(0.8, 1.5)
                except Exception as e:
                    self.log(f"方法 2 失败: {e}")
            ...
            if not allow_clicked:
                try:
                    ...
                    if allow_btn.count() > 0:
                        self.log("点击 Allow 授权应用 (has-text)...")
                        allow_btn.wait_for(state="visible", timeout=10000)
                        allow_btn.click(timeout=5000, force=True)
                        allow_clicked = True
                        self._human_sleep(0.8, 1.5)
                except Exception as e:
                    self.log(f"方法 3 失败: {e}")
            
            if not allow_clicked:
                self.log("警告: 未能点击 Allow 按钮，尝试继续...")
                # 记录当前页面状态以便调试
                try:
                    buttons = page.locator('button').all_text_contents()
                    self.log(f"当前页面按钮: {buttons}")
                    page.screenshot(path="allow_button_not_found.png")
                    self.log("已保存截图: allow_button_not_found.png")
                except Exception:
                    pass
