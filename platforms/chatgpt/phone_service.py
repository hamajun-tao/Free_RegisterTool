...
def _prioritize_united_states(country_slugs: list[str]) -> list[str]:
    ordered = [slug for slug in country_slugs if slug]
    if "united-states" not in ordered:
        return ordered
    return ["united-states", *[slug for slug in ordered if slug != "united-states"]]
...
class SMSToMePhoneService:
    def __init__(self, config: Optional[dict] = None, log_fn: Optional[Callable[[str], None]] = None):
        ...
        self.country_slugs = _prioritize_united_states(
            parse_country_slugs(self.config.get("smstome_country_slugs"))
        )
