...
class RequestHeaderEnhancer:
    ...
    def enhance_headers(
        ...
        Args:
            ...
            locked_version: 锁定的 Chrome 版本号（来自 SessionFingerprint），确保一致性
        ...
        # 添加 Sec-CH-UA 完整链（支持版本锁定）
        headers.update(self._build_sec_ch_ua_chain(locked_version=locked_version))
    ...
    def _build_sec_ch_ua_chain(self, *, locked_version: Optional[str] = None) -> Dict:
        """构建完整的 Sec-CH-UA 链
        
        Args:
            locked_version: 锁定的 Chrome 完整版本号。如果提供，所有 Sec-CH-UA 头
                           将使用此版本号，确保整个 session 内版本一致。
                           如果为 None，则使用原有的随机微调逻辑（向后兼容）。
        """
        # Chrome 136 示例
        sec_ch_ua = '"Chromium";v="136", "Google Chrome";v="136", "Not.A/Brand";v="99"'
        
        if locked_version:
            # 使用锁定的版本号 —— 保证 session 内一致
            sec_ch_ua_full_version_list = (
                f'"Chromium";v="{locked_version}", '
                f'"Google Chrome";v="{locked_version}", '
