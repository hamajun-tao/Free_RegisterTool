...
class ChatGPTRegistrationContext:
    ...
    max_retries: int
    extra_config: dict
    task_control: object = None
    pre_oauth_auto_pay_hook: Optional[Callable[[object, dict], Optional[dict]]] = None
...
class BaseChatGPTRegistrationModeAdapter(ABC):
    ...
    def _build_account_extra(self, result) -> dict:
        return {
            ...
            **self._build_metadata_account_extra(result),
        }

    def _build_metadata_account_extra(self, result) -> dict:
        metadata = getattr(result, "metadata", None) or {}
        if not isinstance(metadata, dict):
            return {}

        extra = {}
        if metadata.get("proxy_geo_country"):
            extra["proxy_geo_country"] = metadata.get("proxy_geo_country")
        auto_pay = metadata.get("auto_pay")
        if isinstance(auto_pay, dict) and auto_pay:
            if auto_pay.get("plan"):
                extra["auto_pay_plan"] = auto_pay.get("plan")
            if auto_pay.get("state"):
                extra["auto_pay_state"] = auto_pay.get("state")
            if auto_pay.get("receipt_url"):
                extra["auto_pay_receipt"] = auto_pay.get("receipt_url")
            if auto_pay.get("flow_order"):
                extra["auto_pay_flow_order"] = auto_pay.get("flow_order")
            if auto_pay.get("error"):
                extra["auto_pay_error"] = auto_pay.get("error")
            if auto_pay.get("idempotency_key"):
                extra["auto_pay_idempotency_key"] = auto_pay.get("idempotency_key")
            geo_diagnostics = auto_pay.get("geo_diagnostics")
            if isinstance(geo_diagnostics, dict) and geo_diagnostics:
                extra["auto_pay_geo_diagnostics"] = geo_diagnostics

        auth = metadata.get("pre_oauth_auth")
        if isinstance(auth, dict) and auth:
            if auth.get("cookie_header"):
                extra["cookie_header"] = auth.get("cookie_header")
            if auth.get("device_id"):
                extra["oai_device_id"] = auth.get("device_id")
        return extra
...
class RefreshTokenChatGPTRegistrationAdapter(BaseChatGPTRegistrationModeAdapter):
    ...
    def _create_engine(self, context: ChatGPTRegistrationContext):
        ...
        return RefreshTokenRegistrationEngine(
            ...
            pre_oauth_auto_pay_hook=context.pre_oauth_auto_pay_hook,
        )
