...
class LuckMailHttpClient:
    ...
    Args:
        ...
        proxy_url: 代理 URL，如 "http://127.0.0.1:7897"
    """
    ...
    def __init__(
        ...
        timeout: float = 30.0,
        use_hmac: bool = False,
        impersonate: Optional[str] = None,  # LuckMail 不需要模拟指纹，默认关闭以避免代理 TLS 问题
    ...
    def _get_sync_session(self) -> requests.Session:
        ...
        if self._sync_session is None:
            ...
            self._sync_session.trust_env = False
    ...
    async def _async_request(
        ...
        try:
            if method.upper() == "GET":
                async with session.get(url, headers=headers, timeout=self.timeout, proxy=self.proxy_url) as response:
                    content = await response.read()
                    return self._parse_response(response.status, content)
            elif method.upper() == "POST":
                async with session.post(url, headers=headers, json=json_data or {}, timeout=self.timeout, proxy=self.proxy_url) as response:
                    content = await response.read()
                    return self._parse_response(response.status, content)
            elif method.upper() == "PUT":
                async with session.put(url, headers=headers, json=json_data or {}, timeout=self.timeout, proxy=self.proxy_url) as response:
                    content = await response.read()
                    return self._parse_response(response.status, content)
            elif method.upper() == "DELETE":
                async with session.delete(url, headers=headers, timeout=self.timeout, proxy=self.proxy_url) as response:
                    content = await response.read()
                    return self._parse_response(response.status, content)
            else:
        ...
        try:
            if method.upper() == "GET":
                ...
                response = session.post(
                    url, headers=headers, json=json_data or {}, timeout=self.timeout
                )
            elif method.upper() == "PUT":
