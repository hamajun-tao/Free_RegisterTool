...
from services.chatgpt_sync import update_account_model_cliproxy_sync, update_account_model_sub2api_sync

router = APIRouter(prefix="/actions", tags=["actions"])


def _build_register_config() -> RegisterConfig:
    all_cfg = config_store.get_all()
    proxy = str(all_cfg.get("proxy_url") or "").strip() or None
    return RegisterConfig(proxy=proxy, extra=all_cfg)
...
def _clear_stale_sub2api_sync_fields(extra: dict) -> None:
    sync_statuses = extra.get("sync_statuses")
    if not isinstance(sync_statuses, dict):
        return
    sub2api = sync_statuses.get("sub2api")
    if not isinstance(sub2api, dict) or sub2api.get("uploaded"):
        return
    sub2api.pop("uploaded_at", None)
    if sub2api.get("skipped") or not sub2api.get("remote_account_id"):
        sub2api.pop("remote_account_id", None)
...
def _apply_action_result(
    ...
    if platform == "chatgpt":
        ...
        elif action_id in ("sync_cliproxyapi_status", "sync_sub2api_status"):
    ...
    if isinstance(result.get("account_extra_patch"), dict):
        ...
        if platform == "chatgpt":
            _clear_stale_sub2api_sync_fields(extra)
        acc_model.set_extra(extra)
        from datetime import datetime, timezone
        acc_model.updated_at = datetime.now(timezone.utc)
        session.add(acc_model)
...
def _resolve_batch_accounts(platform: str, body: BatchActionRequest, session: Session) -> tuple[list[AccountModel], list[int]]:
    ...
    elif body.status_group == "success":
        query = query.where(AccountModel.status != "invalid")
    elif body.status_group == "failed":
        query = query.where(AccountModel.status == "invalid")
    if body.email:
...
def _execute_batch_sub2api_sync(
    ...
    failed_count = 0
    for acc_model in accounts:
        sync_account = build_chatgpt_sync_account(acc_model)
        sync_result = local_sub2api_account_sync_result(sync_account) or query_sub2api_account(
            acc_model.email,
            api_url=api_url,
            api_key=api_key,
        )
        ...
        if ok:
            ...
            failed_count += 1
        summary = (
            f"remote={sync_result.get('remote_state') or 'unknown'}, "
            f"found={'yes' if sync_result.get('found') else 'no'}"
        )
        items.append(
            {
                "id": acc_model.id,
                "email": acc_model.email,
                "ok": ok,
                "message": f"Sub2API status sync complete: {summary}",
                "status": acc_model.status,
            }
        )
    return {
...
def list_actions(platform: str):
    ...
    instance = PlatformCls(config=_build_register_config())
...
def execute_batch_action(
    ...
    if platform == "chatgpt" and action_id == "sync_sub2api_status":
        batch_result = _execute_batch_sub2api_sync(accounts, session, body.params)
        if missing_ids:
            for missing_id in missing_ids:
                batch_result["failed"] += 1
                batch_result["total"] += 1
