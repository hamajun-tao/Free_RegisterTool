...
def classify_local_probe_state(probe: dict[str, Any] | None) -> str:
    ...
    if auth_state == "connection_error":
...
_CONNECTIVITY_ONLY_REASONS = frozenset({"remote_unreachable", "remote_server_error", "local_connection_error"})
...
def apply_chatgpt_status_policy(
    ...
    *,
    ...
    if reason and reason not in _CONNECTIVITY_ONLY_REASONS:
