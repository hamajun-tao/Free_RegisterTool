...
def record_sub2api_sync_result(extra: dict[str, Any], sync_result: dict[str, Any]) -> dict[str, Any]:
    ...
    incoming = dict(sync_result or {})
    now = _utcnow_iso()
    state.update(incoming)
    state["last_attempt_ok"] = bool(incoming.get("ok"))
    state["last_message"] = str(incoming.get("message") or "")
    state["last_attempt_at"] = now
    state["uploaded"] = bool(incoming.get("uploaded"))
    if incoming.get("uploaded"):
        state["uploaded_at"] = now
    else:
        state.pop("uploaded_at", None)
        if incoming.get("skipped") or not incoming.get("remote_account_id"):
            state.pop("remote_account_id", None)

    sync_statuses[SUB2API_SYNC_NAME] = state
    extra["sync_statuses"] = sync_statuses
    return state
...
def persist_sub2api_sync_result(account: Any, sync_result: dict[str, Any]) -> None:
    if isinstance(account, AccountModel) and account.id is not None:
        with Session(engine) as session:
            row = session.get(AccountModel, account.id)
            if row:
                update_account_model_sub2api_sync(row, sync_result, session=session, commit=True)
                return

    extra = getattr(account, "extra", None)
    if isinstance(extra, dict):
        record_sub2api_sync_result(extra, sync_result)
...
def backfill_chatgpt_account_to_cpa(
    ...
    *,
    ...
    sync_account = build_chatgpt_sync_account(account)
    probe_proxy = _get_config_value("proxy_url") or None
    if not probe_proxy:
        try:
            from core.proxy_pool import proxy_pool
            probe_proxy = proxy_pool.get_next(respect_cooldown=False) or None
        except Exception:
            probe_proxy = None
    probe = probe_local_chatgpt_status(sync_account, proxy=probe_proxy)
