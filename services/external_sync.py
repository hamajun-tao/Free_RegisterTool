...
def sync_account(account) -> list[dict[str, Any]]:
    ...
    if platform == "chatgpt":
        ...
        if sub2api_url and sub2api_key:
            from platforms.chatgpt.sub2api_upload import sync_sub2api_account

            sync_result = sync_sub2api_account(
                upload_account,
                api_url=sub2api_url,
                api_key=sub2api_key,
            )
            persist_sub2api_sync_result(account, sync_result)
            results.append({
                "name": "Sub2API",
                "ok": bool(sync_result.get("ok")),
                "msg": str(sync_result.get("message") or ""),
            })
