...
export async function apiFetch(path: string, opts?: RequestInit) {
  ...
  if (method === 'GET') {
    const existing = pendingGetRequests.get(path)
    if (existing) return existing
  }

  const request = doApiFetch(path, opts)
  if (method === 'GET') {
    pendingGetRequests.set(path, request)
    request.finally(() => pendingGetRequests.delete(path))
  }
  return request
}

async function doApiFetch(path: string, opts?: RequestInit) {
